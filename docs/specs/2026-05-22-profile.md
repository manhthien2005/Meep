...
| Profile_Diary tab | `573:3213` | Cùng layout nhưng tab Diary active — M2: hiện mock diary content |
...
6. **Diary tab** — tab bấm được; hiển thị mock diary content (M2); toggle grid/list là tính năng M3
...
Taskbar → tap icon User (active)
    → Profile screen
        ...
        ├─ Tab [book-heart] → Diary tab — hiển thị mock diary content (M2); toggle grid/list là tính năng M3
...
Profile → [Chỉnh sửa]
    → Edit profile screen (list settings)
        ...
        ├─ Tap "Tiểu sử" → text input sheet (multiline, max 150 chars)
        │   Info icon (ⓘ) bên cạnh label → tap hiện inline tooltip: "Tiểu sử của bạn sẽ hiển thị với bạn bè của bạn. Phần giới thiệu bản thân được giới hạn tối đa 150 chữ."
        │   (Figma Frame 3 — tooltip inline trong EditProfileScreen, không phải hint ở bottom sheet)
...
> **Không có** nút Edit trên FriendProfileScreen — hiện 2 nút: **[Nhắn tin]** (navigate to chat) và **[Chia sẻ trang cá nhân]** (mở ShareProfileSheet với title "Chia sẻ liên kết của bạn bè").
...
Profile grid → tap ảnh
    → Photo detail screen:
        ...
        ├─ Thời gian đăng — hiển thị riêng bên dưới ảnh (ví dụ: "17:03") — chỉ giờ, không kèm tên người dùng

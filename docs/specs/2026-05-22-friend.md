...
> Filter bản thân: nếu `result.uid == currentUid` → **[CẬP NHẬT 2026-06-01]** hiển thị card với nhãn **"Đây là tôi"** (thay vì ẩn hoàn toàn). Lý do: user gõ đúng username của mình thì thấy phản hồi rõ ràng hơn là "không tìm thấy". Vẫn không có nút "Thêm".

---

### Luồng chia sẻ invite link

> **[CẬP NHẬT 2026-06-01]** Mọi kênh share (Messenger / Instagram / SMS) route qua **1 system share sheet** của `share_plus` thay vì per-app deep-link intent. Lý do: per-app intent trên Android không đáng tin cậy (app đích có thể ignore). User tự chọn app trong share sheet OS.

```
Bottom sheet → Section "Chia sẻ liên kết Meep của bạn"
    ├─ "Liên kết của bạn" → copy link https://meep.app/invite/{currentUid}
    │       (icon link → icon check 2s sau khi copy)
    ├─ "Messenger"        → Share.share(link)  ─┐
    ├─ "Tin nhắn Instagram" → Share.share(link) ─┼─ cùng 1 system share sheet
    ├─ "Tin Instagram"    → Share.share(link)  ─┘
    └─ "Tin nhắn"         → Share.share(link)  (không còn mock toast)
```
...
| OQ5 | Invite deep link technology | **[CẬP NHẬT 2026-06-01] HTTPS link `https://meep.app/invite/{uid}`** — share qua `share_plus` system share sheet (user tự chọn app: Messenger/Instagram/SMS...). Lý do đổi từ `meep://`: link https hiển thị thân thiện khi paste vào chat + có thể fallback web landing page. **Cần task riêng:** cấu hình Android App Links (`assetlinks.json` trên domain `meep.app`) để link mở thẳng app; receiver flow (`/invite/:uid` handler + `pendingInviteUid`) vẫn là TODO. Router giữ route `/invite/:uid`. |
...
## Implementation deltas vs spec gốc (cập nhật 2026-06-01, leader ThienPDM)

> Các thay đổi cố ý trong quá trình implement (branch `friend-ui`). Đã chốt bởi leader, ghi lại để dev sau trace được contract thật.

| # | Điểm | Spec gốc | Thực tế đã ship | Lý do |
|---|---|---|---|---|
| D1 | `FriendRequestRepository.sendFriendRequest` | `sendFriendRequest(String receiverId)` | `sendFriendRequest({required String senderUid, required String receiverUid})` | Truyền cả senderUid để repo idempotent-guard query không phụ thuộc auth singleton; dễ test. |
| D2 | `FriendRequestRepository` thêm method | — | `Stream<List<FriendRequest>> watchSentRequests(String senderUid)` + `Future<void> acceptFriendRequest(String requestId)` | `watchSentRequests` drive state "Đã gửi" survive re-search/reopen; `acceptFriendRequest` gọi CF callable (client không write trực tiếp). |
| D3 | `FriendState` fields | friends, pendingRequests, searchResult, searchQuery, isLoading, errorMessage | + `sentRequests`, `friendsInitialized` | `sentRequests` cho nút "Đã gửi"; `friendsInitialized` tránh race search-result-trước-friends-stream (nút nhấp nháy "Thêm"→"Bạn bè"). |
| D4 | Self-search result | Ẩn (`uid == currentUid` → bỏ qua) | Hiện card nhãn "Đây là tôi" | Phản hồi rõ hơn là "không tìm thấy". |
| D5 | Share invite | Per-app deep-link intent (Messenger/IG/SMS) + "Tin nhắn" mock toast | Tất cả → 1 `share_plus` system share sheet | Per-app intent Android không đáng tin. |
| D6 | Invite link scheme | `meep://invite/{uid}` | `https://meep.app/invite/{uid}` | Xem OQ5. Cần task Android App Links. |
| D7 | Lỗi mutation (max-20, network) | Toast | Hiển thị `errorMessage` cố định ở header sheet | Đơn giản hoá M2; toast cân nhắc M3. |
| D8 | `sendFriendRequest` khi đã có pending | — (spec không nói) | No-op idempotent (return) + throw `ValidationError` chỉ khi self-request | Chặn spam tap tạo doc trùng. Enforce tuyệt đối vẫn cần CF/rules (đã flag). |
| D9 | Controller resilience | — | Self-healing re-subscribe 3 stream khi snapshot stream error (backoff 3s) | Firestore stream terminate vĩnh viễn khi permission-denied; tự re-listen để UI tự hồi phục. |

### Còn TODO (ngoài scope branch `friend-ui`)
- `FriendFilterDropdown` (feed filter, spec §Luồng FriendsButton dropdown) — vẫn là stub `TODO(F/T10/KhoaLND)`.
- Receiver flow deep link `/invite/:uid` + `pendingInviteUid` — chưa implement.
- Android App Links (`assetlinks.json`) cho domain `meep.app` — cần để https link mở thẳng app.

...
- Là user, tôi muốn xem và bỏ chặn các tài khoản đã chặn, có bước xác nhận trước khi bỏ chặn.
...
| Tài khoản bị chặn 1 (rỗng) | `572:4490` | Empty state — "Không có tài khoản bị chặn nào" |
| Tài khoản bị chặn 2 (danh sách) | `572:4839` | Danh sách — avatar + username + nút "Bỏ chặn" |
| Tài khoản bị chặn 3 (xác nhận) | `1441:3341` | Dialog xác nhận bỏ chặn — "Bạn có chắc muốn bỏ chặn...?" + [Huỷ]/[Xác nhận] |
...
Settings sheet → tap "Tài khoản đã chặn"
    ...
    → Tap [Bỏ chặn]:
        → hiện UnblockConfirmDialog (Figma 1441:3341 — state 3):
            Title: "Bạn có chắc muốn bỏ chặn tài khoản này không?"
            Body:  "Tài khoản này sẽ được gỡ khỏi danh sách chặn. Việc bỏ chặn
                    sẽ không tự động khôi phục trạng thái bạn bè trước đây."
            [Huỷ] | [Xác nhận] (đỏ)
        → [Xác nhận]:
            → unblockUser() — xoá /blocks/{me}_{targetUid}
            → đồng thời xoá /friendships/{pairId} nếu tồn tại
            → user bị xoá khỏi danh sách ngay lập tức
            → nếu danh sách rỗng → empty state (Figma 572:4490 — state 1)
        → [Huỷ]: đóng dialog, giữ nguyên danh sách
```
...
| OQ7 | Unblock có cần confirm dialog không? | UX | 🔄 **Revised (2026-05-31, NganTNK):** CÓ. Thêm `UnblockConfirmDialog` (Figma `1441:3341` — state 3) trước khi gọi `unblockUser()`. Thay đổi so với quyết định ban đầu (bỏ chặn ngay, không confirm) — tránh thao tác nhầm, nhất quán với logout/delete. Đã cập nhật §Luồng Tài khoản đã chặn + Happy path + user story. |
...
| Unblock | Settings → Tài khoản đã chặn → [Bỏ chặn] → confirm dialog → [Xác nhận] → xoá block + friendship; danh sách rỗng → empty state |

...
- evidence: grep `^describe\(` trong `firestore.rules.test.ts` hits 9 describes — 8 collection-specific (`/users/{uid}`, `/users/{uid} — field validation`, `/usernames/{username}`, `/posts/{postId}`, `/friend_requests/{requestId}`, `/blocks/{blockId}`, `/diary/{entryId}`, `/conversations/{conversationId}`) + 1 (`security invariants`). Total 908 lines. Collections COVERED tốt. Collections MISSING test: `/friendships/{pid}`, `/spaces/{spaceId}`, `/space_members/{spaceId}/members/{uid}`, `/posts/{postId}/reactions/{reactorUid}`, `/users/{uid}/notifications/{notifId}`, `/users/{uid}/fcmTokens/{tokenId}`, `/users/{uid}/private/{docId}`, `/users/{uid}/feed/{postId}` (partial — only "owner can read/cannot write" trong `/users` describe, không có cross-uid test). Storage tests `storage.rules.test.ts` cover `/avatars/{uid}` + `/diary/{uid}` describes nhưng MISSING `/posts/{uid}` describe entirely.
- access_path: regression risk — rule edit (vd thêm field whitelist cho /posts update fix POST-SEC-001) đã có /posts test cover. NHƯNG: thêm rule cho /spaces/Space members hay /friendships fix FRIEND-SEC-001 sẽ KHÔNG có test catch → silent breaking change. Production rule có thể drift khỏi spec ở collections chưa tested.
- risk: testing coverage gap; không phải data leak hiện tại nhưng amplify P0/P1 fix risk cho collections chưa tested
- fix: thêm 5 describe block trong firestore.rules.test.ts (PRIORITY ORDER theo P0-P3 fix impact):
  1. `/friendships/{pid}` — pair với FRIEND-SEC-001 fix
  2. `/spaces/{spaceId}` — sau khi spaces có sửa rule
  3. `/space_members/{spaceId}/members/{uid}` — verify member read OK, non-member deny
  4. `/posts/{postId}/reactions/{reactorUid}` — pair với REACTION-SEC-001 fix; verify create + author read
  5. `/users/{uid}/notifications`, `/users/{uid}/fcmTokens`, `/users/{uid}/private` subcollections — owner-only matrix
  Thêm 1 describe block trong storage.rules.test.ts: `/posts/{uid}/` — pair với STORAGE-001 fix verify (sau khi STORAGE-001 tighten friend boundary)
...
- test: `firebase/functions/package.json:scripts.test:rules` chạy cả 2 file; sau fix CI `pr-check.yml:154-160` sẽ catch regression — NHƯNG chỉ enforce sau khi CI-SEC-001 fix xóa `|| echo warning` fallback (đã có dependency declared bên dưới)
- deps: depends on (verify after) POST-SEC-001, CHAT-SEC-001/002, STORAGE-001, STORAGE-002, FRIEND-SEC-001, REACTION-SEC-001 — viết test xanh trước, sửa rule, test đỏ → re-write, test xanh. Blocked-by CI-SEC-001 (nếu CI-SEC-001 không fix, test coverage không enforce được).
...
pass_7_reverify_notes (deep audit per user request — go as kỹ as possible round 2):
- SCOPE: full read 10 CF files chưa cover ở pass-6 + 2 client modules còn lại (chat conversation, notification) + existing test files (firestore.rules.test.ts, storage.rules.test.ts, index.test.ts) + infra (firebase.json, firestore.indexes.json, AndroidManifest.xml, develop-staging.yml, assetlinks.json).
- NO NEW SECURITY ISSUES sau pass-7 — đây là dấu hiệu confidence cao hơn vì pass-6 đã catch hết các gap visible. Pattern "mỗi pass + nhiều issues" của pass-2 → pass-6 đã hội tụ.
- FACT FIX TESTING-SEC-001 (P2): evidence "6 collections" + "/conversations missing" SAI. Verified 9 describes (8 collection-specific + 1 invariants), 908 lines. `/conversations` ĐÃ có test. Missing thực sự: `/friendships`, `/spaces`, `/space_members`, `/posts/reactions`, `/users/notifications`, `/users/fcmTokens`, `/users/private`. Storage: missing `/posts/{uid}` block (chỉ avatars + diary cover). Fix scope updated với priority order theo P0-P3 dependency.
- CF AUDIT 10 files: hết clean. updateSpace zod refine reject empty patch (line 27-36) → good. kickMember/leaveSpace/transferOwnership đều có creator + member check. createSpace verify friendship trước batch (line 78-101). onSpaceDeleted transition guard null→non-null. spacePostFanOut handle deleted Space gracefully. onMessageCreated narrow type. _fcm prune permanent token errors. _helpers + onFriendRequestAccepted defensive type narrowing. KHÔNG có new gap.
- CLIENT AUDIT 2 modules (chat + notification): firebase_conversation_repository sendMessage line 188-192 set `lastSenderId: senderId` từ client (CHAT-SEC-002 đã cover). getOrCreateConversation client-side fallback (CHAT-SEC-001 cover). markAsRead dùng dot-notation `lastReadAt.$uid` — **CHAT-SEC-002 fix cần verify rule check `lastReadAt is map` xử lý đúng dot-notation update** (note for fix implementation). firebase_notification_repository saveFcmToken stores token plaintext trong `/users/{uid}/fcmTokens/{tokenId}.token` (SHA-256 doc ID). Rule `isOwner(uid)` only access — defensible per Firebase FCM token storage pattern. markAsRead single-field `read: true` mirror rule whitelist.
- INFRA AUDIT:
  - firebase.json: predeploy hooks run lint+test+build (line 33-37) — defense before deploy. Emulator ports declared. OK.
  - firestore.indexes.json: 10 composite indexes, không có field PII. **OBSERVATION (note-only, non-security)**: posts indexes có `spaceId` singular (line 23-35) NHƯNG schema dùng `spaceIds` plural. Có thể legacy/dead index — sweep candidate cho cleanup PR riêng.
  - AndroidManifest.xml: verify PERM-001 evidence — chỉ `INTERNET` permission. `RECEIVE_BOOT_COMPLETED` + `POST_NOTIFICATIONS` MISSING confirmed. CAMERA + READ_MEDIA_IMAGES có thể auto-declared qua image_picker/camera plugin merged manifest — note-only.
  - assetlinks.json EXISTS với SHA256 cert fingerprint cho `dev.meep.meep`. **MINOR OBSERVATION (note-only)**: single fingerprint, không separate cho debug vs release. Khi release production keystore khác debug, autoVerify deep link có thể fail trên release build — leader cần update fingerprint trước M3.
  - develop-staging.yml: paths filter (line 18-23) save CI minutes — smart. `if: false` deploy jobs đến khi fill secrets — defensible.
- COUNT TALLY: pass-6 total 22 → pass-7 total 22 (no escalation). Distribution unchanged: P0=2, P1=6, P2=7, P3=7.
- COVERAGE FULL: 7/7 client modules (rollcall không tồn tại), 12/12 CF files (vs pass-6 10/12), 4 test files, 5 infra files (firebase.json, indexes, manifest, develop-staging, assetlinks).
- VERDICT confirm: not_ready. AUTH-SEC-002 + USER-SEC-001 P0/P1 pipeline still blocker. Recommended path 11 must-fix giữ nguyên.
...
Final distribution after pass-7 reverify (deep audit round 2): **P0=2, P1=6, P2=7, P3=7 — total 22 issues UNCHANGED**. Pass-7 không escalate issues mới — confirm pass-6 catch hết visible gaps. Fact fixes: TESTING-SEC-001 evidence corrected (9 describes thực tế, không phải 6; `/conversations` ĐÃ cover; missing thực sự là /friendships /spaces /space_members /reactions /notifications /fcmTokens /private + storage /posts). Coverage upgrade: 7/7 client modules + 12/12 CF files + 4 test files + 5 infra files. NO NEW SECURITY GAP across 10 CF files + 2 client + tests + infra deep read.

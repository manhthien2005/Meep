...
Files:
- `lib/features/settings/presentation/blocked_accounts_page.dart` — empty/list blocked users (Figma `572:4490` empty, `572:4839` list)
- `lib/features/settings/presentation/unblock_confirm_dialog.dart` — dialog xác nhận bỏ chặn (Figma `1441:3341` — state 3), dùng lại `showAppConfirmDialog`
...
- `test/features/settings/blocked_accounts_page_test.dart` — widget tests: list hiện đúng, tap "Bỏ chặn" → confirm dialog → [Xác nhận] xoá khỏi danh sách / [Huỷ] giữ nguyên; danh sách rỗng → empty state

Acceptance criteria:
- [ ] `BlockedAccountsPage` load stream `watchBlockedUsers()` — list avatar + username + "Bỏ chặn" button
- [ ] Tap "Bỏ chặn" → hiện `UnblockConfirmDialog` (Figma `1441:3341`) → [Xác nhận] → `unblockUser()` + xoá friendship → user bị xoá khỏi danh sách ngay; danh sách rỗng → empty state. [Huỷ] → giữ nguyên (⚠️ logic unblock+unfriend blocked by OQ5 — chờ ThienPDM quyết)

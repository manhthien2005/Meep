...
class _CameraSectionState extends ConsumerState<CameraSection> {
  ...
  Future<void> _onCapture() async {
    ...
    if (camState.mode == CameraMode.single) {
      unawaited(
        context.push('/capture-preview', extra: CapturePreviewArgs.single(imagePath: path)),
      );
      return;
    }
  ...
  Future<void> _onAlbumPick() async {
    ...
    if (image != null && mounted) {
      unawaited(
        context.push('/capture-preview', extra: CapturePreviewArgs.single(imagePath: image.path)),
      );
    }
  }
...
class _DualSlot extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cornerRadius =
        isPip ? AppProportions.pipCornerRadius : 0.0;

#!/usr/bin/env pwsh
# Update auth sub-issues (#20-#26) to add "Files phải đọc trước (context)" section.
# Inserts the section before "## Acceptance criteria" in each issue body.
# Idempotent: skips issues that already have the section.
...
foreach ($num in ($issueMap.Keys | Sort-Object)) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  ✗ Failed to fetch issue #$num" -ForegroundColor Red
        continue
    }

    $body = ($bodyJson | ConvertFrom-Json).body

    if ($body -match "Files phải đọc trước") {
        Write-Host "  ⚠  Section already exists — skipping" -ForegroundColor Yellow
        continue
    }

    if ($body -notmatch [regex]::Escape($insertBeforePattern)) {
        Write-Host "  ✗ Cannot find insertion point '$insertBeforePattern' in #$num — skipping" -ForegroundColor Red
        continue
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  ✗ Failed to update #$num" -ForegroundColor Red
    }
}
